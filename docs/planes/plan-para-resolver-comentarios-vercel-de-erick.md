...
- 📍 **Corrección post-resolved (2026-07-06)**: al marcar threads como resolved, el API reveló el campo `selection` (ausente en el listado) — los dos "eliminar" de normas seleccionaban **"de Casa"**, satisfechos por el rename global. Se restauró el párrafo DC-3 sobre-eliminado (Codex lo había señalado en e01-r01) y JJQY4eQgY8p5 pasó de pendiente a resuelto. Total final: **45 resueltos / 24 pendientes**, los 45 marcados resolved en Vercel.
- ❌ **No ejecutados en esta etapa** (24 threads, decisión registrada en snapshot y registro): 6 de imágenes (archivos identificados; requieren candidatas + aprobación visual de Luis), 7 de layout/preguntas (propuestas documentadas; requieren decisión de Luis/Erick), 11 viejos no realizados por criterio de Luis.
