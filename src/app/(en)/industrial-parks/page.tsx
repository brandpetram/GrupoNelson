      ...
      <section className="pt-32 pb-14 md:pt-40 md:pb-20 border-b border-border">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-8 text-sm text-muted-foreground">
            <span><span className="font-semibold text-foreground">4</span> parks</span>
            <span className="text-border hidden sm:inline">·</span>
            <span><span className="font-semibold text-foreground">75+</span> hectares</span>
            <span className="text-border hidden sm:inline">·</span>
            <span><span className="font-semibold text-foreground">28+</span> established companies</span>
          </div>
        </div>
      </section>

      {/* Park sections — orden invertido: Nelson II primero */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 px-10">
              ...
              <div className="px-6 py-8 lg:px-12 xl:px-16 lg:py-8 flex flex-col justify-center">
                ...
                {(park.availableLandM2 ?? 0) > 0 && (
                  <p className="text-sm text-muted-foreground mb-4">
                    <span className="font-medium text-foreground">
                      {new Intl.NumberFormat('en-US').format(park.availableLandM2!)} m²
                    </span>{' '}
                    of available land
                  </p>
                )}
                {(park.availableAreaM2 ?? 0) > 0 && (
                  <p className="text-sm text-muted-foreground mb-4">
                    <span className="font-medium text-foreground">
                      {new Intl.NumberFormat('en-US').format(park.availableAreaM2!)} m²
                    </span>{' '}
                    of available building space
                  </p>
                )}
