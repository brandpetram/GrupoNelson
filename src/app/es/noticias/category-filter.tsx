...
export const BlogFilter = ({ categories, posts }: BlogFilterProps) => {
    ...
    const activeCategory = pathname === '/noticias' ? 'all' : pathname.split('/noticias/category/')[1]?.split('/')[0] || 'all'

    const handleClick = (slug: string) => {
        if (slug === 'all') router.push('/noticias')
        else router.push(`/noticias/category/${slug}`)
    }
